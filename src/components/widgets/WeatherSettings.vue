		...
		getModalHTML() {
			return `
				<div class="modal-header">
					<h3>${t('dashy', 'Weather Settings')}</h3>
					<button class="close-btn" type="button" title="Close">✕</button>
				</div>
				<form class="settings-form">
					<div class="form-group">
						<label for="location">${t('dashy', 'Location')}</label>
						<input 
							type="text" 
							id="location" 
							placeholder="${t('dashy', 'Enter location...')}"
							value="${this.localSettings.location}"
						>
					</div>
					<div class="form-group">
						<label for="unit">${t('dashy', 'Temperature unit')}</label>
						<select id="unit">
							<option value="C" ${this.localSettings.unit === 'C' ? 'selected' : ''}>${t('dashy', 'Celsius (°C)')}</option>
							<option value="F" ${this.localSettings.unit === 'F' ? 'selected' : ''}>${t('dashy', 'Fahrenheit (°F)')}</option>
						</select>
					</div>
					<div class="form-group">
						<label class="checkbox-label">
							<input type="checkbox" id="showDetails" ${this.localSettings.showDetails ? 'checked' : ''}>
							<span>${t('dashy', 'Show detailed information')}</span>
						</label>
					</div>
